...
EJEMPLO 14-3 LUIS ENRIQUE GARCÍA REYES
Presentado por: 
...
E_c = 25000*1000 # [kN/m²]
h_entrepiso = 3
D_losa =    16000/9           # kgf/m²
D_cub =     D_losa          # kgf/m²
Lu_dirY = np.array([6,6])     # [m] Luces en dirección Y
Lu_dirX = np.array([6,6])     # [m] Luces en dirección X
...
datos_portico_X = {
    'xnod' : np.array([
        ...
        [0, 15],
        [0, 18],
        [6, 0],   # coordenadas de cada nodo [x, y]
        [6, 3],
        [6, 6],
        [6, 9],
        [6, 12],
        [6, 15],
        [6, 18],
        [12, 0],   # coordenadas de cada nodo [x, y]
        [12, 3],
        [12, 6],
        [12, 9],
        [12, 12],
...
K_c_x = matriz_rigidez_portico(datos_portico_X, True) # kN/m
...
th_eta, th_U, th_F, th_V, th_Mom, th_U_mod, th_F_mod = ADLTH(K_c_x,
                            Phi_x, 
                            np.sqrt(mme_x),
                            Japon2011, 
                            ome_x, 
...
ax1.plot(Japon2011[:,0], th_eta.T, label=[f'Modo {i+1}' for i in range(N_PISOS)])
ax1.set_title('Respuesta de las ecuaciones desacopladas')
ax1.set_xlabel('Tiempo (s)')
ax1.set_ylabel(r'$\eta$')
    
ax2.plot(Japon2011[:,0], th_U.T, label=[f'Piso {i+1}' for i in range(N_PISOS)])
ax2.set_title('Desplazamiento')
ax2.set_xlabel('Tiempo (s)')
ax2.set_ylabel('U (m)')

ax3.plot(Japon2011[:,0], th_F.T, label=[f'Modo {i+1}' for i in range(N_PISOS)])
ax3.set_title('Fuerzas')
ax3.set_xlabel('Tiempo (s)')
ax3.set_ylabel('\F (kN)')
fig.suptitle(f'SISMO del Centro')
